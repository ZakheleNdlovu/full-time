...
const TeamDetails2 = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            const response = await fetch(`https://site.api.espn.com/apis/site/v2/sports/soccer/ger.1/teams/${team.id}/roster`)
            if (!response.ok) {
        ...
        const getStats = async () => {
            const response = await fetch(`http://sports.core.api.espn.com/v2/sports/soccer/leagues/ger.1/seasons/2025/types/1/teams/${team.id}/record?lang=en&region=us`)
            if (!response.ok) {
