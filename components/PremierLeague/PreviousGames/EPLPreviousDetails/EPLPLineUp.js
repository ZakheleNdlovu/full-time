...
import EPLPTeamPlayer from './EPLPTeamPlayer'

const EPLPLineUps = ({ game }) => {
    ...
    try {
        useEffect(() => {
            const getStarters = async () => {
                const response = await fetch(`https://sports.core.api.espn.com/v2/sports/soccer/leagues/eng.1/events/${game.id}/competitions/${game.id}/competitors/${game.competitions[0].competitors[0].team.id}/roster`)
                if (!response.ok) {
            ...
            const getStarters2 = async () => {
                const response = await fetch(`https://sports.core.api.espn.com/v2/sports/soccer/leagues/eng.1/events/${game.id}/competitions/${game.id}/competitors/${game.competitions[0].competitors[1].team.id}/roster`)
                if (!response.ok) {
                    throw new Error("error");
                        ...
                        starters.map((player, i) => {
                            return (
                                <View key={i} style={{ width: '100%' }}>
                                    <View >
                                        <EPLPTeamPlayer id={player.playerId} playerDet={player} />
                                    </View>
                                </View>
                            )
                        })
