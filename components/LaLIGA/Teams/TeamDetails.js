...
const TeamDetails1 = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            const response = await fetch(`https://site.api.espn.com/apis/site/v2/sports/soccer/esp.1/teams/${team.id}/roster`)
            if (!response.ok) {
        ...
        const getStats = async () => {
            const response = await fetch(`http://sports.core.api.espn.com/v2/sports/soccer/leagues/esp.1/seasons/2025/types/1/teams/${team.id}/record?lang=en&region=us`)
            if (!response.ok) {
