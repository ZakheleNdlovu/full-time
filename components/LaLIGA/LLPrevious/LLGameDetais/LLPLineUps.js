...
import LLTeamPlayer from '../../Home/LLGameDetails/LLTeamPLayer'

const LLPLineUps = ({ game }) => {
    ...
    try {
        useEffect(() => {
            const getStarters = async () => {
                const response = await fetch(`https://sports.core.api.espn.com/v2/sports/soccer/leagues/esp.1/events/${game.id}/competitions/${game.id}/competitors/${game.competitions[0].competitors[0].team.id}/roster`)
                if (!response.ok) {
            ...
            const getStarters2 = async () => {
                const response = await fetch(`https://sports.core.api.espn.com/v2/sports/soccer/leagues/esp.1/events/${game.id}/competitions/${game.id}/competitors/${game.competitions[0].competitors[1].team.id}/roster`)
                if (!response.ok) {
                    throw new Error("error");
